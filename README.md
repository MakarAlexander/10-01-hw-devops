# Домашнее задание к занятию "Системы мониторинга" "Макарцев Александр Владимирович"
...
6. Какие из ниже перечисленных систем относятся к push модели, а какие к pull? А может есть гибридные?
    ...
    - Nagios
#### Решение
- Prometheus - гибрид, изначально pull-модель, доступен push с помощью Pushgateway;
- TICK - push модель;
- Zabbix - гибрид, работает по pull и push модели, в зависимости от режима агента (Active-Passive);
- VictoriaMetrics - изначально это долгосрочное хранилище для Prometheus, поэтому работало по модели Push (remote_write). Однако есть vmagent, который умеет сам собирать метрики (pull), а также принимать мтерики извне (Push) и передавать на хранение в VM-кластер. Но push-модель взаимодействия с компонентами Victoriametrics это не поменяло;
- Nagios - push модель, использует агенты, которые установлены на компонентах для отслеживания и пушат данные на Nagios Server;
#
